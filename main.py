...
max = 45000 # Max storage (Free)
premium = 80000 # Max storage (Premium)
...
@bot.command()
async def help(ctx):
    if ctx.guild.id == 1054138859073589310:
        invitemenu = discord.Embed(title="Invite Me",description="Click [here](https://discord.com/api/oauth2/authorize?client_id=1059093188729638972&permissions=8&scope=bot) to invite me to your own server",color=color)
        ...
        helpmenu.description = "For support, join the discord [here](https://discord.gg/novastock)"
...
async def status():
	await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name=f"{len(bot.guilds)}x Server!"))
# gen command
            ...
            if str(ctx.message.attachments) != "[]":
                ...
                if a.status_code != 200:
                    ...
                    with open(f"Guilds/{ctx.guild.id}.json","w") as file:
                        json.dump(data,file,indent=4)
                    if added == 0:
                        success = discord.Embed(title="No Lines Added",description=f"Max storage reached!\n\nTo get more storage, join the discord [here](https://discord.gg/novastock)",color=color)
                    else:
                        success = discord.Embed(title="Lines Added",description=f"Lines added - {added}\n\nCheck new stock with `.stock`",color=color)
                    await ctx.send(embed=success)
            else:
...
# Discord server I discord.gg/novasell
